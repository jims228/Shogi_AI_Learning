...
export default function BishopTrainingPage() {
  ...
  const currentLesson = BISHOP_LESSONS[currentStepIndex];
  ...
  const handleMove = useCallback((move: { from?: { x: number; y: number }; to: { x: number; y: number }; piece: string; drop?: boolean }) => {
    // ShogiBoard側で既に成った状態の駒(piece)が送られてくるので、
    // checkMoveでは座標だけでなく、成っているかどうかも確認できる（今回は座標判定のみでOK）
  ...
  const nextButton = isCorrect ? (
    ...
    >
      {currentStepIndex < BISHOP_LESSONS.length - 1 ? "次のステップへ" : "レッスン完了！"}
        ...
        <AutoScaleToFit minScale={0.7} maxScale={1.45} className="w-full h-full">
          <WoodBoardFrame paddingClassName="p-3" className="inline-block">
            <ShogiBoard
              ...
              orientation="sente"
              showPromotionZone={currentLesson.step === 2}
              autoPromote={currentLesson.step === 2}
            />
          </WoodBoardFrame>
        </AutoScaleToFit>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm font-bold text-slate-400">
        ...
        <span className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
          <span
            ...
            style={{ width: `${((currentStepIndex + 1) / BISHOP_LESSONS.length) * 100}%` }}
          />
        </span>
        <span>{BISHOP_LESSONS.length}</span>
      </div>
  ...
  if (isMobileWebView) {
    return (
      <MobileLessonShell
        ...
        explanation={
          <MobileCoachText
            tag={`STEP ${currentStepIndex + 1}/${BISHOP_LESSONS.length}`}
  ...
  return (
    <LessonScaffold
      ...
      progress01={(currentStepIndex + 1) / BISHOP_LESSONS.length}
