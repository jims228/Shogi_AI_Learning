...
export default function TarefuTrainingPage() {
  ...
  const currentLesson = PAWN_LESSON_2_TAREFU_STEPS[currentStepIndex];
  ...
  const handleNext = () => {
    if (currentStepIndex < PAWN_LESSON_2_TAREFU_STEPS.length - 1) setCurrentStepIndex((p) => p + 1);
    else {
  ...
  const nextButton = isCorrect ? (
    ...
    >
      {currentStepIndex < PAWN_LESSON_2_TAREFU_STEPS.length - 1 ? "次のステップへ" : "レッスン完了！"}
      ...
      <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
        ...
        <span className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
          <span
            ...
            style={{ width: `${((currentStepIndex + 1) / PAWN_LESSON_2_TAREFU_STEPS.length) * 100}%` }}
          />
        </span>
        <span>{PAWN_LESSON_2_TAREFU_STEPS.length}</span>
      </div>
  ...
  if (isMobileWebView) {
    ...
    return (
      <MobileLessonShell
        ...
        explanation={
          <MobileCoachText
            tag={`STEP ${currentStepIndex + 1}/${PAWN_LESSON_2_TAREFU_STEPS.length}`}
  ...
  return (
    <LessonScaffold
      ...
      topLabel="DRILL"
      progress01={(currentStepIndex + 1) / PAWN_LESSON_2_TAREFU_STEPS.length}
