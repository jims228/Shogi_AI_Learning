...
export default function Tsume1Lesson001Page() {
  ...
  const currentLesson = TSUME_1_LESSONS[currentStepIndex];
  ...
  const initializeBoard = useCallback(() => {
    ...
    try {
      ...
      // 持ち駒の手動パース（既存パーサーの制限回避）
      // SFENに "b G" (先手の持ち駒に金) が含まれている場合
  ...
  if (isMobileWebView) {
    ...
    return (
      <MobileLessonShell
        ...
        explanation={
          <MobileCoachText
            tag={`TSUME ${currentStepIndex + 1}/${TSUME_1_LESSONS.length}`}
  ...
  return (
    <div className="min-h-screen bg-[#f6f1e6] text-[#2b2b2b] flex flex-col">
      ...
      <header className="h-16 border-b border-black/10 bg-white/50 flex items-center px-4 justify-between sticky top-0 z-10 backdrop-blur-sm">
        ...
        <div className="font-bold text-lg text-[#3a2b17]">1手詰：第1問</div>
        <div className="w-20" />
      </header>
          ...
          <div className="flex items-center gap-2 text-sm font-bold text-slate-400">
            ...
            <span className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
              <span 
                ...
                style={{ width: `${((currentStepIndex + 1) / TSUME_1_LESSONS.length) * 100}%` }}
              />
            </span>
            <span>{TSUME_1_LESSONS.length}</span>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-black/5">
            ...
            {isCorrect && (
              <div className="animate-in fade-in zoom-in-95 duration-300">
                ...
                >
                  {currentStepIndex < TSUME_1_LESSONS.length - 1 ? "次のステップへ" : "レッスン完了！"}
