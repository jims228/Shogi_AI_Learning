...
const formatScore = (score?: EngineMultipvItem["score"]) => {
  if (!score) return "-";
  if (score.type === "mate") {
    // mateがundefinedの場合は0として扱う（または安全な処理）
...
const getQualityBadge = (diff: number | null | undefined) => {
  ...
  if (diff <= -400) return <span className="px-1.5 py-0.5 text-[10px] rounded bg-red-600 text-white font-bold">大悪手</span>;
  if (diff <= -200) return <span className="px-1.5 py-0.5 text-[10px] rounded bg-orange-500 text-white font-bold">悪手</span>;
  if (diff <= -80) return <span className="px-1.5 py-0.5 text-[10px] rounded bg-yellow-500 text-white font-bold">疑問</span>;
  if (diff >= 200) return <span className="px-1.5 py-0.5 text-[10px] rounded bg-blue-500 text-white font-bold">好手</span>;
